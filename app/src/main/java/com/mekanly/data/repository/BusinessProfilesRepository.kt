...
class BusinessProfilesRepository {
    ...
    fun getSimilarBusinessProducts(id: Long, callback: (ResponseBodyState) -> Unit) {
        callback(ResponseBodyState.Loading)
        apiService.getSimilarBusinessProducts(id)
            .enqueue(object : Callback<ResponseDataList<BusinessCategory>> {
                override fun onResponse(
                    call: Call<ResponseDataList<BusinessCategory>>, response: Response<ResponseDataList<BusinessCategory>>
                ) {
                    if (response.isSuccessful) {
                        val businessProfiles = response.body()?.data ?: emptyList()
                        callback(ResponseBodyState.SuccessList(businessProfiles))
