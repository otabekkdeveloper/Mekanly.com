...
import com.mekanly.data.models.UserName
...
class BottomSheetComments : BottomSheetDialogFragment() {
        ...
        binding.commentEditText.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                ...
                if (hasText) {
                    ...
                    binding.sendButton.setImageResource(R.drawable.ic_unselected_arrow_to_top) // неактивная иконка
                }
            }
    ...
    private fun handleCommentAction(newCommentText: String) {
        ...
        val newComment = Comment(
            user = UserName(
                id = 0,
                username = AppPreferences.getUsername() ?: "Unknown User" // ИСПРАВЛЕНО: добавлено AppPreferences.getUsername()
            ),
            ...
            userId = 0,
            isOwner = true,
            commentableId = args.houseId,
            commentableType = COMMENT_TYPE_HOUSE,
            parentId = null,
            userReaction = null,
            replies = emptyList()
        )
        ...
        if (::commentAdapter.isInitialized) {
            ...
            commentAdapter = CommentsAdapter(
                requireContext(),
                mutableListOf(newComment),
                onDeleteComment = { commentId, position ->
                    // Вызываем ViewModel для удаления комментария
                    viewModel.deleteComment(commentId)
                    // Удаляем из адаптера после успешного ответа от сервера
                    commentAdapter.removeItem(position)
                },
                onUpdateComment = { commentId, position, newText ->
                    val updateCommentBody = UpdateCommentBody(newText)
                    viewModel.updateComment(commentId, updateCommentBody)

                    // ИСПРАВЛЕНИЕ: Проверяем, что элемент существует
                    commentAdapter.getItem(position)?.let { currentComment ->
                        val updatedComment = currentComment.copy(comment = newText)
        ...
        viewModel.addComment(
            ...
            // Колбэк для обработки результата добавления комментария
            // Здесь можно обработать успешное добавление или ошибку
        }
    }
    ...
    private fun showPopupMenu(view: View) {
        val commentPopup = PopupMenu(requireContext(), view, Gravity.END or Gravity.BOTTOM)
