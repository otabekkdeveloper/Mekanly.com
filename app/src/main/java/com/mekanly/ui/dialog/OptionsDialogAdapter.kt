...
class OptionsDialogAdapter(
    ...
    inner class OpportunitiesViewHolder(private val binding: ItemOpportunityDialogBinding) :
        ...
        fun bind(item: Option) {
            // Получаем enum по id вместо имени
            val possibilityEnum = Possibilities.fromId(item.id)

            val context = binding.textView.context
            binding.textView.text = when (possibilityEnum?.id) {
                ...
                else -> item.name // Если id не найден, используем оригинальное имя
            }

            val iconRes = when (possibilityEnum?.id) {
