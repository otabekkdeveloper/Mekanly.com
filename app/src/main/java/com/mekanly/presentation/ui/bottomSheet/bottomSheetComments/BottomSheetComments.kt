...
class BottomSheetComments : BottomSheetDialogFragment() {
            ...
            viewModel.commentsState.collectLatest {
                when(it){
                    ...
                    is ResponseBodyState.SuccessList -> {
                        binding.progressBar.visibility = View.GONE
                        setCommentsAdapter(it.dataResponse as List<DataComments>)
                        if (it.dataResponse.isEmpty()){
                            showErrorSnackBar(requireContext(), binding.root,"Empty Data")
                        }
                    }
    ...
    private fun setCommentsAdapter(list:List<DataComments>){
